...
/* RSVP meal preference: maximum two choices */
function updateMealPreferenceLimit(form) {
  ...
  choices.forEach(choice => {
    choice.disabled = selected >= 2 && !choice.checked;
  });
}

document.addEventListener('change', function (event) {
  if (!event.target.matches('.meal-preferences input[type="checkbox"][name="meal"]')) return;
