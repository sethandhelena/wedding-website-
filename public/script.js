...
async function initRegistry(){
  ...
  grid.classList.add('registry-data-loading');
  ...
  let items=[];
  try{const res=await fetch('/api/registry'+(store?`?store=${encodeURIComponent(store)}&_fresh=${Date.now()}`:`?_fresh=${Date.now()}`),{cache:'no-store',headers:{'Cache-Control':'no-cache'}});const data=await res.json();if(!res.ok)throw new Error();items=data.items||[];if(items.length)renderRegistryItems(grid,items,!!store)}catch{items=Array.from(grid.querySelectorAll('.gift-card')).map((el,i)=>({el,index:i,store:el.dataset.store,status:el.dataset.status}))}
...
function freshManagedImageUrl(url){
  ...
  // Local design assets are versioned with the deployment and do not need cache busting.
  if(/^assets\//i.test(raw) || /^\/assets\//i.test(raw))return raw;

  const joiner=raw.includes('?')?'&':'?';
  return `${raw}${joiner}_fresh=${Date.now()}`;
}
...
async function initEditablePageContent(){
  ...
  const finish=()=>{
    document.querySelectorAll('[data-content-bg]').forEach(el=>{
      if(!el.classList.contains('admin-image-ready')){
        el.style.removeProperty('background-image');
        el.classList.add('admin-image-ready');
      }
    });
    document.documentElement.classList.remove('admin-content-loading');
  };
  ...
  try{
    const res=await fetch(`/api/pages/${encodeURIComponent(page)}?_fresh=${Date.now()}`,{
      cache:'no-store',
...
async function initJournalPost(){
  ...
  try{
    ...
    const placesToStayGallery = p.slug==='places-to-stay-near-willmar' ? `
