...
function initRsvp(){
  ...
  form.addEventListener('submit',async e=>{e.preventDefault();const submit=form.querySelector('[type="submit"]');const old=submit.textContent;submit.disabled=true;submit.textContent='Sending…';if(msg)msg.textContent='';try{const body=Object.fromEntries(new FormData(form).entries());const res=await fetch('/api/rsvp',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const data=await res.json();if(!res.ok)throw new Error(data.error||'Unable to submit RSVP.');if(msg)msg.textContent=data.message||'Thank you! Your RSVP has been received.';form.reset()}catch(err){if(msg)msg.textContent=err.message}finally{submit.disabled=false;submit.textContent=old}})
}
...
async function initJournal(){
  ...
  let posts=[];try{const res=await fetch('/api/journal');const data=await res.json();if(!res.ok)throw new Error();posts=data.posts||[];if(posts.length)grid.innerHTML=posts.map((p,i)=>`<article class="post-card${i>=6?' journal-extra':''}"><div class="post-image ${escapeAttr(p.image_class||'')}"${p.image_url?` style="background-image:url('${escapeAttr(p.image_url)}')"`:''}></div><div class="post-content"><p class="post-date">${escapeHtml([p.post_date,p.category].filter(Boolean).join(' · '))}</p><h3>${escapeHtml(p.title)}</h3><p>${escapeHtml(p.excerpt||'')}</p><a href="#">Read More</a></div></article>`).join('')}catch{}
  const button=document.getElementById('journalLoadMore');if(!button)return;let expanded=false;function render(){document.querySelectorAll('.journal-extra').forEach(p=>{p.hidden=!expanded;p.style.display=expanded?'':'none'});button.textContent=expanded?'Show Fewer Journals':'View More Journals';button.setAttribute('aria-expanded',expanded?'true':'false')}button.addEventListener('click',()=>{expanded=!expanded;render()});render();
}
