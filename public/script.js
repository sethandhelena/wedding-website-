...
function initRsvp(){
    ...
    return `
      <section class="household-guest-card" data-household-guest="${index}">
        ...
        ${isPrimary
          ...
          : `<label><span data-content="form_guest_name_label">Guest Name</span>
        ...
        <div class="household-guest-row">
          <label><span data-content="form_guest_type_label">Guest Type</span>
            <select name="household_guest_${n}_type" data-household-type="${index}">
              <option value="Adult" selected>Adult</option>
              <option value="Child 7+">Child 7+</option>
              <option value="Child Under 7">Child Under 7 — Kids Meal</option>
            </select>
          </label>

          <fieldset class="household-meal-fieldset">
            <legend data-content="form_meal_label">Meal Choice</legend>
            <div class="household-meal-options" data-standard-meals="${index}">
            ...
            <div class="kids-meal-selected" data-kids-meal="${index}" hidden>
              Kids Meal
              <input type="radio" name="household_guest_${n}_meal" value="Kids Meal" data-household-meal="${index}" data-kids-meal-input="${index}">
            </div>
          </fieldset>
        </div>
      </section>`;
  ...
  function syncKidsMealForCard(card){
    if(!card)return;
    const type=card.querySelector('[data-household-type]')?.value||'Adult';
    const index=card.dataset.householdGuest;
    const standard=card.querySelector(`[data-standard-meals="${index}"]`);
    const kidsWrap=card.querySelector(`[data-kids-meal="${index}"]`);
    const kidsInput=card.querySelector(`[data-kids-meal-input="${index}"]`);

    const isUnder7=type==='Child Under 7';

    if(standard)standard.hidden=isUnder7;
    if(kidsWrap)kidsWrap.hidden=!isUnder7;

    if(isUnder7){
      card.querySelectorAll('[data-household-meal]').forEach(input=>{
        input.checked=input===kidsInput;
      });
      if(kidsInput)kidsInput.checked=true;
    }else if(kidsInput?.checked){
      kidsInput.checked=false;
    }
  }

  function syncAllKidsMeals(){
    guestsWrap?.querySelectorAll('[data-household-guest]').forEach(card=>syncKidsMealForCard(card));
  }
  ...
  function renderHouseholdGuests(){
    ...
    for(let i=0;i<count;i++){
      ...
      if(prior){
        ...
        syncKidsMealForCard(card);
        if(prior.meal && prior.meal!=='Kids Meal'){
  ...
  function setHouseholdRequiredState(){
    ...
    guestsWrap?.querySelectorAll('[data-household-guest]').forEach((card,index)=>{
      ...
      const kidsInput=card.querySelector('[data-kids-meal-input]');

      if(name && index>0)name.required=accepted;

      meals.forEach(meal=>meal.required=false);

      if(accepted){
        if(type==='Child Under 7'){
          if(kidsInput){
            kidsInput.checked=true;
            kidsInput.required=true;
          }
        }else{
          const standardMeals=card.querySelectorAll('[data-standard-meals] [data-household-meal]');
          standardMeals.forEach(meal=>meal.required=true);
        }
      }
    });
  }
  ...
  guestsWrap?.addEventListener('change',e=>{
    if(e.target.matches('[data-household-type]')){
      syncKidsMealForCard(e.target.closest('[data-household-guest]'));
      setHouseholdRequiredState();
    }
  });
  ...
  form.addEventListener('submit',async e=>{
    ...
    try{
      ...
      body.guests=count;
      body.meal=accepted
        ? household.map(g=>`${g.name}${g.type!=='Adult'?` (${g.type})`:''} — ${g.meal}`).join('; ')
        : '';
