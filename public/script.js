...
function loadEditableBackground(el,url){
  ...
  image.onerror=()=>{
    // If an uploaded image is unavailable, fall back gracefully.
  ...
  try{
    ...
    document.querySelectorAll('[data-content-bg]').forEach(el=>{
      ...
      if(url){
        ...
        // No replacement saved: restore the original CSS fallback only
        // after page content has finished loading.
        el.style.removeProperty('background-image');
      }
    });
  }catch{}
