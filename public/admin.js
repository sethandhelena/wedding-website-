...
async function loadRsvps(){
  ...
  $('#rsvpRows').innerHTML=d.items.map(r=>{
    ...
    const guestList=household
      ? `<div class="admin-household-list">
          ${household.guests.map(g=>`
            <div class="admin-household-guest compact">
              <strong>${escapeHtml(g.name)}</strong>
              <span class="admin-household-meta">${escapeHtml(g.type||'Guest')}${g.meal?` · ${escapeHtml(g.meal)}`:''}</span>
            </div>`).join('')}
          ${household.note?`<div class="admin-household-note"><strong>Note:</strong> ${escapeHtml(household.note)}</div>`:''}
        </div>`
      : `<div>
          <strong>${escapeHtml(r.name)}</strong>
          ${r.meal?`<div class="admin-household-meta">${escapeHtml(r.meal)}</div>`:''}
          ${r.message?`<div class="admin-household-note">${escapeHtml(r.message)}</div>`:''}
        </div>`;
