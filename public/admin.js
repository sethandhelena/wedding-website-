...
const PAGE_SCHEMAS={
  ...
  wedding:{label:'Wedding',file:'wedding.html',sections:[
    ...
    ['Travel & Stay',[
      ['travel_eyebrow','Eyebrow'],
      ['travel_title','Heading'],
      ['stay_card_title','Places to stay title'],
      ['stay_card_text','Places to stay text','textarea'],
      ['stay_card_button','Places to stay button'],
      ['directions_title','Directions title'],
      ['directions_text','Directions text','textarea'],
      ['directions_button','Directions button'],
      ['directions_link','Directions link'],
      ['things_title','Things to do title'],
      ['things_text','Things to do text','textarea'],
      ['things_button','Things to do button'],
      ['things_link','Things to do link']
    ]],
  ...
  rsvp:{label:'RSVP',file:'rsvp.html',sections:[
    ...
    ['RSVP Form',[
      ['form_name_label','Primary guest name label'],
      ['form_email_label','Household email label'],
      ['form_attendance_label','Attendance label'],
      ['form_household_count_label','Household guest-count label'],
      ['form_household_title','Guest details heading'],
      ['form_household_text','Guest details instructions','textarea'],
      ['form_guest_name_label','Additional guest name label'],
      ['form_guest_type_label','Guest type / kids meal label'],
      ['form_meal_label','Meal choice label'],
      ['form_message_label','Song request / note label']
    ]],
...
function renderPageEditor(slug){
  ...
  if(slug==='wedding'){
    const stayCardDefaults={
      stay_card_title:'Places to Stay',
      stay_card_text:'We’ve gathered a few hotel and vacation-rental recommendations near Willmar for guests who need a place to stay.',
      stay_card_button:'View Stay Options →'
    };
    for(const [k,v] of Object.entries(stayCardDefaults)){
      if(!(k in values)) values[k]=v;
    }
  }
  ...
  if(slug==='rsvp'){
    const rsvpDefaults={
      ...
      form_attendance_label:'Will you be joining us?',
      form_household_count_label:'How many people are included in your RSVP?',
      form_household_title:'Guest Details',
      form_household_text:'Please include everyone in your party, both adults and children. Only one email address is needed per household.',
      form_guest_name_label:'Guest Name',
      form_guest_type_label:'Guest Type',
      form_meal_label:'Meal Choice',
      form_message_label:'Song Request or Note'
    };
...
function parseHouseholdRsvpMessage(message){
  const raw=String(message||'');
  if(!raw.startsWith('HOUSEHOLD RSVP'))return null;

  const parts=raw.split(/\n\nNOTE\n/);
  const guestPart=parts[0]||'';
  const notePart=parts.slice(1).join('\n\nNOTE\n');
  const lines=guestPart.split('\n').slice(1).filter(Boolean);

  const guests=lines.map(line=>{
    const cleaned=line.replace(/^\d+\.\s*/,'');
    const bits=cleaned.split('|').map(s=>s.trim());
    return {
      name:bits[0]||'',
      type:bits[1]||'',
      meal:bits[2]||''
  ...
  return {guests,note:notePart.trim()};
}

async function loadRsvps(){
  const d=await api('/api/admin/rsvps');

  $('#rsvpRows').innerHTML=d.items.map(r=>{
    const household=parseHouseholdRsvpMessage(r.message);

    const householdHtml=household
      ? `<div class="admin-household-list">
          ${household.guests.map(g=>`
            <div class="admin-household-guest">
              <strong>${escapeHtml(g.name)}</strong>
              <span>${escapeHtml(g.type||'Guest')}</span>
              ${g.meal?`<span>${escapeHtml(g.meal)}</span>`:''}
            </div>`).join('')}
          ${household.note?`<div class="admin-household-note"><strong>Note:</strong> ${escapeHtml(household.note)}</div>`:''}
        </div>`
      : `${r.message?`<br><small>${escapeHtml(r.message)}</small>`:''}`;

    return `<tr>
      <td><strong>${escapeHtml(r.name)}</strong>${householdHtml}</td>
      <td>${escapeHtml(r.attendance)}</td>
      <td>${escapeHtml(r.guests)}</td>
      <td>${household
        ? household.guests.map(g=>`${escapeHtml(g.name)} — ${escapeHtml(g.meal||'—')}`).join('<br>')
        : escapeHtml(r.meal||'')}</td>
...
async function loadJournal(){
  ...
  journalPosts=d.items||[];

  const special={"id": "", "title": "Places to Stay Near Willmar", "category": "Travel", "post_date": "September 2, 2026", "excerpt": "A few hotel and vacation-rental recommendations to help make your wedding weekend in Willmar comfortable, convenient, and easy to plan.", "content": "There isn’t a required hotel block for our wedding. We wanted everyone to have the flexibility to choose the stay that works best for their plans and budget, so we gathered a few nearby options to make planning your weekend a little easier.", "image_class": "", "image_url": "/assets/places-to-stay-willmar-hotel-hero.jpg", "sort_order": 0, "published": 1, "_special_static": true};

  const hasPlacesToStay=journalPosts.some(i=>
    String(i.title||'').trim().toLowerCase()==='places to stay near willmar'
  );

  if(!hasPlacesToStay)journalPosts=[...journalPosts,special];

  $('#journalList').innerHTML=journalPosts.map(i=>`<article class="admin-item">
    <div>
      <p class="eyebrow">${escapeHtml(i.category||'Journal')} · ${i.published?'Published':'Draft'}</p>
      <h3>${escapeHtml(i.title)}</h3>
      <p>${escapeHtml(i.post_date||'')}</p>
      <p>${escapeHtml(i.excerpt||'')}</p>
      ${i._special_static?'<p><small>Website article — edit and save to add it to your Journal database.</small></p>':''}
    </div>
    <div class="item-actions">
      <button type="button" data-edit-journal="${escapeAttr(i.id||'places-to-stay-static')}" class="secondary">Edit</button>
      ${i._special_static?'':`<button type="button" data-delete-journal="${i.id}" class="danger">Delete</button>`}
    </div>
  </article>`).join('');

  $$('[data-edit-journal]').forEach(b=>b.onclick=()=>{
    const key=b.dataset.editJournal;
    const item=key==='places-to-stay-static'
      ? journalPosts.find(i=>i._special_static)
      : journalPosts.find(i=>String(i.id)===String(key));
    editJournal(item);
  });

  $$('[data-delete-journal]').forEach(b=>b.onclick=()=>deleteJournal(b.dataset.deleteJournal));
}
